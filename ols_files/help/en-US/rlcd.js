...
const $RLCD = (function () {
  ...
  _RLCD.U_speak       = U_Speak;          // function function U_Speak(P_szText, P_rVolume, P_rPitch, P_rRate, P_szLang, P_szName);
  _RLCD.U_Shutup      = U_ShutUp;         // function U_ShutUp()
  _RLCD.F_fEnaSpeach  = U_fEnaSpeach;     // function U_fEnaSpeach(P_fEnaSpeach)
  _RLCD.F_fInit       = F_fInitialized;   // function F_fInitialized();
...
function U_Init_Speak()
{
  ...
  for (i = 0; i < S_aVoices.length ; i++) {
    var Voice0 = S_aVoices[i];  
    var lang = Voice0.lang;
    if (lang == S_szLang) {
      Voice0.default = true;
      S_Utterance.voice = Voice0
      S_Utterance.lang  = Voice0.lang;
      break; 
    } /* if */
  } /* for */
...
{
  try {      
    var aElem = document.getElementsByClassName('Cl_Hide_Mobile');
